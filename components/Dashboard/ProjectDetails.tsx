                        ...
                        <div className='flex justify-between'>
                            ...
                            <div className='flex gap-4'>
                                <div className='self-center'>
                                    {
                                        isWatching ? (
                                            <RemoveRedEyeIcon onClick={() => {setIsWatching(!isWatching)}}/>
                                        ) : (
                                            <RemoveRedEyeOutlinedIcon onClick={() => {setIsWatching(!isWatching)}}/>
                                        )
                                    }
                                </div>
                                <div className='self-center'>
                                {
                                        isLiked ? (
                                            <FavoriteIcon onClick={() => {setIsLiked(!isLiked)}} sx={{ color: pink[500] }}/>
                                        ) : (
                                            <FavoriteBorderOutlinedIcon onClick={() => {setIsLiked(!isLiked)}}/>
                                        )
                                    }
                                </div>
                                <div className='p-2 bg-gray-800 rounded-3xl'>
                                    <span className='text-xs text-white'><ContentCopyIcon fontSize='small' className='cursor-pointer' /> {projectId.substring(0, 15)}...</span>
                                </div>
                            </div>
                        </div>
                    </div>
